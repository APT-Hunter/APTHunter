...
for FILE in $topic/*; do
	java -Dlog4j.debug=true -Dlog4j.configuration=nochecker.log4j.properties -cp .:target/kafkaclients-1.0-SNAPSHOT-jar-with-dependencies.jar com.bbn.tc.services.kafka.FileConsumer file:///$FILE -np -psf ../ta5-serialization-schema/avro/TCCDMDatum.avsc -csf ../ta5-serialization-schema/avro/TCCDMDatum.avsc -rg -call -co earliest -cdm -c -roll 5000000 -wj -d 10000000 $args;
done
