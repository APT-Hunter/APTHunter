...
with driver.session() as session:
	...
	for index, compromised_process in enumerate(compromised_process_list):
		...
		# Establish FootHold
		result = session.run("""MATCH p=(n1)-[r:SYSCALL*1..3]->(n2)-[r2:SYSCALL]->(n3) WHERE n1.name = $process_condition AND r2.type =~'EXECUTE|FORK|CLONE' AND n3.caption =~ '/bin/bash' AND r2.timestamp >= $timestamp_condition 
	...
	WHERE n1.name = $process_condition AND r2.type =~'EXECUTE|FORK|CLONE' AND n3.caption =~ '/sbin/.*|/bin/.*|/usr/bin/.*|/usr/local/.*|/usr/sbin/.*' AND not n3.caption =~ '/bin/bash' AND r2.timestamp >= $timestamp_condition 
	...
	### Privilage escalation using exploits of vulnerable services (e.g., load_helper.ko)
		...
		result = session.run("""MATCH p=(n1)-[r:SYSCALL*1..]->(n2)-[r2:SYSCALL]->(n3) 
	WHERE r2.type =~ 'START_USER' AND n1.name = $process_condition AND r2.timestamp >= $timestamp_condition 
	RETURN n3.host as host, n1.caption as caption,  n2.caption as caption_n2, r2.type as syscall_2, n3.caption as caption_n3, r2.timestamp as timestamp, count(n3) as count""", timestamp_condition = initial_comp_timestamp, process_condition = compromised_process)
		for record in result:
			#rel = record["host"]
			#rel = record
			...
			threat_scrore = 5
			certainty_score = 10		
			Priv_Escal_2 = Priv_Escal_2.append(pd.DataFrame({'host':[record["host"]], 'detection_timestamp': ["-"], 'source': [record["caption"]], 'SYSCALL': ["-"], 'Target': [record["caption_n3"]], 'Count': [record["count"]], 'threat_Score': [threat_scrore], 'certainity_Score': [certainty_score]}),ignore_index=True)

		# Lateral Movement
		result = session.run("""MATCH p=(n1)-[r:SYSCALL*1..]->(n2)-[r2:SYSCALL]->(n3) WHERE (n1.name = $process_condition) AND r2.type =~ 'SENDMSG|CONNECT' AND n3.caption STARTS WITH $Trusted_Addresses AND r2.timestamp >= $timestamp_condition 
