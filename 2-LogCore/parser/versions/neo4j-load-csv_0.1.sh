...
MERGE (n1:SUBJECT {name: (split(line[1], ':'))[0] , caption: split(line[1], (split(line[1], ':'))[0] + ':')[1]})
MERGE (n2:SUBJECT {name: (split(line[4], ':'))[0] , caption: split(line[4], (split(line[4], ':'))[0] + ':')[1]})
WITH line,n1,n2
CREATE (n1)<-[:SYSCALL {nodeType:line[2], type : line[2]}]-(n2)
\""
