			...
			with open('./logs/' + log_file) as jsondata:		    
				...
				if (cdm_record_type == "com.bbn.tc.schema.avro.cdm20.Event"):			
					...
					if not (event_type == "EVENT_OPEN" or event_type == "EVENT_EXECUTE" or event_type == "EVENT_READ" or event_type == "EVENT_FORK" or event_type == "EVENT_ACCEPT" or event_type == "EVENT_CONNECT"):
						continue
					if  (event_type == "EVENT_OPEN" and previousEvent == "EVENT_READ" and previousSeq == (cdm_record_values['sequence']['long'] +1)):
					...
					try:
						for key_object_exf in Exfil:
							if (event_type == 'EVENT_EXECUTE'):							
								if (key_object_exf in cdm_record_values['properties']['map']['cmdLine']):
									backward_object_list = []
									backward_object_path_length = []
									print ("key object_exfil:" + key_object_exf)								
									print (cdm_record_values['predicateObjectPath']['string'])
									key_object = cdm_record_values['predicateObject']['com.bbn.tc.schema.avro.cdm20.UUID']
									flag = True
									break
							elif (event_type == 'EVENT_OPEN'):
								if (key_object_exf in cdm_record_values['predicateObjectPath']['string']):						
									backward_object_list = []
									backward_object_path_length = []
									print ("key object_exfil:" + key_object_exf)								
									print (cdm_record_values['predicateObjectPath']['string'])
									...
									break
						if (not flag):
							for key_object_InitComp in InitComp:
								if (event_type == 'EVENT_CONNECT' or event_type == 'EVENT_ACCEPT'):
									if (cdm_record_values['properties']['map']['exec'] == key_object_InitComp):
										backward_object_list = []
										backward_object_path_length = []
									#if (key_object_recon == cdm_record_values['properties']['map']['cmdLine']):
										print ("key object InitComp:" + key_object_InitComp)								
										key_object = cdm_record_values['predicateObject']['com.bbn.tc.schema.avro.cdm20.UUID']
										flag = True
										break
							...
							for key_object_recon in Recon:
								if (event_type == 'EVENT_EXECUTE'):
									if ((cdm_record_values['properties']['map']['cmdLine']).startswith(key_object_recon)):
										...
										backward_object_path_length = []
									#if (key_object_recon == cdm_record_values['properties']['map']['cmdLine']):
										print ("key object Recon:" + key_object_recon)								
										key_object = cdm_record_values['predicateObject']['com.bbn.tc.schema.avro.cdm20.UUID']
										flag = True
										break
						...
						if (not flag):
							for ii in backward_object_list:
								if (ii == cdm_record_values['predicateObject']['com.bbn.tc.schema.avro.cdm20.UUID'] or ii == cdm_record_values['subject']['com.bbn.tc.schema.avro.cdm20.UUID']):
									...
									if (backward_object_path_length[backward_object_list.index(ii)] > 10):								
										backward_object_list.pop(backward_object_list.index(ii))
										backward_object_path_length.pop(backward_object_list.index(ii))
